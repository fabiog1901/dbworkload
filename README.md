...
- At high level of concurrency, the metrics generated by each thread overload the pipe used to communicate with the MainProcess.
As a result, the MainProcess cannot reliably calculate the count and the percentiles of all executed transactions.
This affects the printed stats, which will show far less executions.
  
  As a way to mitigate that, double check the total QPS on the server side (for CockroachDB, using the DBConsole).
  You can alternatively set `--stats-frequency 0` to disable stats entirely.
