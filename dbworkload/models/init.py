...
def init(
    ...
    # PG or CRDB?
    try:
        dbms: str = dbworkload.utils.common.get_dbms(dburl)
    ...
    dburl = dbworkload.utils.common.get_new_dburl(dburl, db)
    if skip_import:
...
def __init_create_schema(
    ...
    dburl = dbworkload.utils.common.get_new_dburl(dburl, db_name)
    ...
    schema_sql_file = os.path.abspath(
        dbworkload.utils.common.get_based_name_dir(workload_path) + ".sql"
    )
...
def __init_generate_data(procs: int, workload_path: str, dbms: str, csv_max_rows: int):
    ...
    load = dbworkload.utils.common.get_workload_load(workload_path)
    if not load:
        logger.info(
    ...
    csv_dir: str = dbworkload.utils.common.get_based_name_dir(workload_path)
    ...
    dbworkload.utils.simplefaker.SimpleFaker(
...
def __init_import_data(
    ...
    csv_dir = dbworkload.utils.common.get_based_name_dir(workload_path)
    load = dbworkload.utils.common.get_workload_load(workload_path)

    # Start the http server in a new Process
    mp.Process(
        target=dbworkload.utils.common.httpserver, args=(csv_dir, 3000), daemon=True
    ).start()
            ...
            with conn.cursor() as cur:
                ...
                for table in load.keys():
                    ...
                    for chunk in chunked_list:
                        if dbms == "CockroachDB":
                            stmt = dbworkload.utils.common.get_import_stmt(
                                chunk,
                                table.replace("__", "."),
